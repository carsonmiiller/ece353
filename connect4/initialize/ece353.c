 ...
 *      None
 *****************************************************/
void ece353_MKII_S2_wait_for_press(void){
    // counter for 5s
    int count = 0;
    while(1){
...
void ece353_MKII_RGB_IO_Init(bool en_primary_function)
{
    ...
    // config Red LED to be output
    P2->DIR |= BIT6;

    // config Green LED to be output
    P2->DIR |= BIT4;

    // config Blue LED to be output
    P5->DIR |= BIT6;



    // selects the Primary module function for all 3 pins
    if(en_primary_function){
        // select primary module function for P2.6 (Red LED)
        P2->SEL1 &= ~BIT6;
        P2->SEL0 |= BIT6;

        // select primary module function for P2.4 (Green LED)
        P2->SEL1 &= ~BIT4;
        P2->SEL0 |= BIT4;

        // select primary module function for P5.6 (Blue LED)
        P5->SEL1 &= ~BIT6;
        P5->SEL0 |= BIT6;
    }
 ...
 *      None
 *****************************************************/
void ece353_MKII_RGB_PWM(uint16_t ticks_period, uint16_t red_num, uint16_t green_num, uint16_t blue_num){
    ...
    // Turn the first TimerA peripheral on
    TIMER_A0->CTL &= ~TIMER_A_CTL_MC_MASK;
    TIMER_A0->CTL |= TIMER_A_CTL_MC__UP | TIMER_A_CTL_CLR | TIMER_A_CTL_SSEL__SMCLK;

    // Turn the second TimerA peripheral on
    TIMER_A2->CTL &= ~TIMER_A_CTL_MC_MASK;
    TIMER_A2->CTL |= TIMER_A_CTL_MC__UP | TIMER_A_CTL_CLR | TIMER_A_CTL_SSEL__SMCLK;
}
...
void ece353_ADC14_PS2_X(void){
    // Configure P6.0 (A15) the X direction as an analog input
    P6->SEL0 |= BIT0;
    P6->SEL1 |= BIT1;

    // config the ADC14 control regs
    ...
    // config memory control reg so that we associate the correct
...
/*****************************************************
 * Busy waits for 500mS and then returns.
 *
 * Timer32_1 MUST be configured as a 16-bit timer.
 * Assume that the MCU clock runs at 3MHz.  You will
 * need to use a pre-scalar in order to achieve a delay
 ...
 *      None
 *****************************************************/
void ece353_T32_1_wait_500mS(void){
    ece353_T32_1_wait_100mS();
    ece353_T32_1_wait_100mS();
    ece353_T32_1_wait_100mS();
    ece353_T32_1_wait_100mS();
