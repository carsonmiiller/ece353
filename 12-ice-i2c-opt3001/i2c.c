...
static __inline void i2c_set_tx_mode(void){
    // setting BIT4 of EUSCI_B1 CTLW0 register sets EUSCI_B1 to Transmitter
    EUSCI_B1->CTLW0 |= BIT4;
}
...
static __inline void i2c_set_rx_mode(void){
    // clearing BIT4 of EUSCI_B1 CTLW0 register sets EUSCI_B1 to Receiver
    EUSCI_B1->CTLW0 &= ~BIT4;
}
...
static __inline uint8_t i2c_wait_for_rx(void){
    while(EUSCI_B_IFG_RXIFG0 == 0){        // busy wait
    }
...
static __inline void i2c_wait_busy(void){
    while(EUSCI_B_STATW_BBUSY & EUSCI_B1->STATW){};
}
